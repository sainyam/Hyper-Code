...
def get_row_prob_back(prob_df, df, row_index, y, theta):
    ...
    relevant_probs = prob_df[
        (prob_df['Y'] == y) &
        (prob_df['Y_value'] > theta)
    ]
    grouped_probs = relevant_probs.groupby('Z').apply(lambda g: g[g['X_value'] == row[g['X']].values[0]]['prob'].prod())
...
def rec_row_prob_back(df, row_indexes, theta, cgm, y):
    ...
    for row_index in row_indexes:
        prob *= get_row_prob_back(prob_df, df, row_index, y, theta)
    return prob
